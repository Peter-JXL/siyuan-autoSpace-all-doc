...
// --- 配置项 ---
const BASE_URL = 'http://127.0.0.1:6806';    // 替换为你的实际地址
const AUTH_TOKEN = 'Token 0b7hbsfmjp1cf99f'; // 替换为你的实际 token
const LOG_DIR = 'logs'; // 日志文件夹名称
const LOG_FILE = path.join(__dirname, LOG_DIR, `app_log_${Date.now()}.log`); // 日志文件路径

const BATCH_SIZE = 200; // 每批处理的 autoSpace 请求数量
...
async function main() {
    ...
    try {
        ...
        // 4. 对每个文档ID发送 autoSpace 请求 (分批处理并暂停)
        log('\n--- 4. 对每个文档ID发送 autoSpace 请求 (分批处理) ---');

        for (let i = 0; i < uniqueDocIds.length; i += BATCH_SIZE) {
            const batchIds = uniqueDocIds.slice(i, i + BATCH_SIZE);
            log(`\n--- 处理批次: ${Math.floor(i / BATCH_SIZE) + 1} (文档ID ${i + 1} 到 ${Math.min(i + BATCH_SIZE, uniqueDocIds.length)}) ---`);

            const batchPromises = batchIds.map(async (docId) => {
                try {
                    log(`正在发送 autoSpace 请求，文档ID: ${docId}`);
                    await postRequest('/api/format/autoSpace', { id: docId });
                    log(`文档ID: ${docId} 的 autoSpace 请求完成。`);
                } catch (error) {
                    log(`文档ID: ${docId} 的 autoSpace 请求失败，跳过此文档。`, 'ERROR');
                }
            });
